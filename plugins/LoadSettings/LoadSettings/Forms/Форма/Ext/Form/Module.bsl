...
Процедура ЗагрузитьНастройкиВРежимеКоманднойСтроки(ОбъектЯдра, СтруктураДействий) Экспорт
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		ЗнОтображатьОтчетAllureВБраузере = ПолучитьЗначениеПереданногоПараметра(СтрутктураJSON, "ОтображатьОтчетAllureВБраузере");
		Если ЗнОтображатьОтчетAllureВБраузере = Истина Тогда
			ОбъектЯдра.ОтображатьОтчетAllureВБраузере = Истина;
		КонецЕсли;	
		...
		ОбъектЯдра.КоманднаяСтрокаОткрытияФичи = ПолучитьЗначениеПереданногоПараметра(СтрутктураJSON, "КоманднаяСтрокаОткрытияФичи");
...
Функция НастройкиСервер(ЭтоLinux)
	...
	Иначе	//значит это первый запуск VB
		НастройкиРезультат = СоздатьСтруктуруПараметровЗапуска();
...
Функция СоздатьСтруктуруПараметровЗапуска()
	...
	Рез.Вставить("КоманднаяСтрокаОткрытияФичи", "code -g %FeatureFile%:%FeatureLine%");
...
Функция КоллекцияКомандЗапуска()
	...
	Рез.Вставить("КоманднаяСтрокаОткрытияФичи", Новый Структура("Р, А", "КоманднаяСтрокаОткрытияФичи", "CmdLineForFeatureOpen"));
