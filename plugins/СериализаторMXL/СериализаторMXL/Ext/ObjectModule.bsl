...
Функция СоздатьДанныеПоТабличномуДокументу(ТабличныйДокументИлиМассивТабличныхДокументов, РежимыЗагрузкиИлиИмяКолонкиЗамещения = Неопределено, ИмяКолонкиЗамещения = Неопределено, ВызываемИзУФ = Ложь) Экспорт
	...
	Если ВызываемИзУФ Тогда
		ЗначениеСоответствияРС = ВернутьЗначениеСоответствия( "РегистрСведенийНаборЗаписей" );
		ЗначениеСоответствияРН = ВернутьЗначениеСоответствия( "РегистрНакопленияНаборЗаписей" );
		ЗначениеСоответствияРБ = ВернутьЗначениеСоответствия( "РегистрБухгалтерииНаборЗаписей" );
		ЗначениеСоответствияРР = ВернутьЗначениеСоответствия( "РегистрРасчетаНаборЗаписей" );
		
		Для каждого Элем Из Данные Цикл
			Стр = Строка(Элем.Значение);
			Если Найти(Стр, ЗначениеСоответствияРС) > 0 Тогда
				ИмяРС = Сред(Стр, СтрДлина( ЗначениеСоответствияРС ) + 2);
				Данные[Элем.Ключ] = ПолучитьКлючиЗаписиРСПоНаборуЗаписейРС(Элем.Значение, ИмяРС);				
			ИначеЕсли Найти(Стр, ЗначениеСоответствияРН) > 0 Тогда
				ИмяРН = Сред(Стр, СтрДлина( ЗначениеСоответствияРН ) + 2);
				Данные[Элем.Ключ] = ПолучитьКлючиЗаписиРНПоНаборуЗаписейРН(Элем.Значение, ИмяРН);
			ИначеЕсли Найти(Стр, ЗначениеСоответствияРБ) > 0 Тогда
				ИмяРБ = Сред(Стр, СтрДлина( ЗначениеСоответствияРБ ) + 2);
				Данные[Элем.Ключ] = ПолучитьКлючиЗаписиРБПоНаборуЗаписейРБ(Элем.Значение, ИмяРБ);
			ИначеЕсли Найти(Стр, ЗначениеСоответствияРР) > 0 Тогда
				ИмяРР = Сред(Стр, СтрДлина( ЗначениеСоответствияРР ) + 2);
				Данные[Элем.Ключ] = ПолучитьКлючиЗаписиРРПоНаборуЗаписейРР(Элем.Значение, ИмяРР);
			КонецЕсли;
		КонецЦикла;
...
Функция ТаблицуВМассивСтруктур(ТаблицаЗначений)
	
	СтрокаКолонок = "";
	Префикс = "";
	Для каждого Колонка из ТаблицаЗначений.Колонки Цикл
		Если НЕ ПустаяСтрока(СтрокаКолонок) Тогда 
			Префикс = ",";
		КонецЕсли;
		СтрокаКолонок = СтрокаКолонок + Префикс + Колонка.Имя;
	КонецЦикла;
...
Функция МассивСтруктурВТаблицу(МассивСтруктур, ТаблицаЗначений)
...
Функция ПреобразоватьМакетДанныхВСтроку(ТабличныйДокумент) Экспорт
	ПротоДанные = Новый Структура;
	
	ПрочитатьДанныеИзМакета(ПротоДанные,ТабличныйДокумент,Неопределено);
	
	ПротоДанныеКопия = Новый Структура;
	Для каждого КлючЗначение из ПротоДанные Цикл
		ПротоДанныеКопия.Вставить(КлючЗначение.Ключ, Новый Структура);
		Для каждого ОписаниеОбъекта из КлючЗначение.Значение Цикл
			
			Если ОписаниеОбъекта.Ключ = "Объект" Тогда 
				Продолжить;
			КонецЕсли;
			
			Значение = ОписаниеОбъекта.Значение;
			Если ОписаниеОбъекта.Ключ  = "Режимы" Тогда
				РежимыНеобязательные = Новый Структура;
				Для каждого Элемент из Значение Цикл
					Если Элемент.Значение Тогда
						РежимыНеобязательные.Вставить(Элемент.Ключ, Элемент.Значение);
					КонецЕсли;
				КонецЦикла;
				Значение = РежимыНеобязательные;
			КонецЕсли;
			...
			Если ТипЗнч(Значение) = Тип("ТаблицаЗначений") Тогда
				Значение = ТаблицуВМассивСтруктур(Значение);
			КонецЕсли;
			Если ОписаниеОбъекта.Ключ = "ТабличныеЧасти" Тогда
				Для каждого ТабличнаяЧасть из ОписаниеОбъекта.Значение Цикл
					ЗначениеТабличнойЧасти = Новый Массив;
					Для каждого Строка из ТабличнаяЧасть.Значение Цикл
						ЗначениеТабличнойЧасти.Добавить(ТаблицуВМассивСтруктур(Строка));
					КонецЦикла;
					Значение[ТабличнаяЧасть.Ключ] = ЗначениеТабличнойЧасти;
				КонецЦикла;
			КонецЕсли;
					 
			ПротоДанныеКопия[КлючЗначение.Ключ].Вставить(ОписаниеОбъекта.Ключ, Значение);
		КонецЦикла;
	КонецЦикла;
	...
	ЗаписатьJSON(Запись, ПротоДанныеКопия);
...
Функция ПолучитьКлючиЗаписиРНПоНаборуЗаписейРН(НаборЗаписей,ИмяРН)
	
	Результат = Новый Массив;
		
	МассивИзмерений = Новый Массив;
	Для каждого Измерение Из Метаданные.РегистрыНакопления[ИмяРН].Измерения Цикл
		МассивИзмерений.Добавить(Измерение.Имя);
	КонецЦикла;
	...
	Для каждого Запись Из НаборЗаписей Цикл
		...
		КлючЗаписи = РегистрыНакопления[ИмяРН].СоздатьКлючЗаписи(ЗначенияКлюча);
...
Функция ПолучитьКлючиЗаписиРБПоНаборуЗаписейРБ(НаборЗаписей,ИмяРБ)
	...
	Для каждого Измерение Из Метаданные.РегистрыБухгалтерии[ИмяРБ].Измерения Цикл
		Если Измерение.Балансовый Тогда
			МассивИзмерений.Добавить(Измерение.Имя);
		Иначе
			МассивИзмерений.Добавить(Измерение.Имя + "Дт");
			МассивИзмерений.Добавить(Измерение.Имя + "Кт");
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого Запись Из НаборЗаписей Цикл
		...
		КлючЗаписи = РегистрыБухгалтерии[ИмяРБ].СоздатьКлючЗаписи(ЗначенияКлюча);
...
Функция ПолучитьКлючиЗаписиРРПоНаборуЗаписейРР(НаборЗаписей,ИмяРР)
	
	Результат = Новый Массив;
		
	МассивИзмерений = Новый Массив;
	Для каждого Измерение Из Метаданные.РегистрыРасчета[ИмяРР].Измерения Цикл
		МассивИзмерений.Добавить(Измерение.Имя);
	КонецЦикла;
	...
	Для каждого Запись Из НаборЗаписей Цикл
		...
		ЗначенияКлюча.Вставить("Регистратор", Запись.Регистратор);		
		ЗначенияКлюча.Вставить("ПериодРегистрации", Запись.ПериодРегистрации);
		ЗначенияКлюча.Вставить("ВидРасчета", Запись.ВидРасчета);
		
		Для каждого Измерение Из МассивИзмерений Цикл
			ЗначенияКлюча.Вставить(Измерение, Запись[Измерение]);
		КонецЦикла;
		
		КлючЗаписи = РегистрыРасчета[ИмяРР].СоздатьКлючЗаписи(ЗначенияКлюча);
...
Функция СоздатьДанныеИзМакетов(МассивТабличныхДокументов, РежимыЗагрузки, ИмяКолонкиЗамещения)
	...
	Для каждого ТабличныйДокумент Из МассивТабличныхДокументов Цикл
		...
		ИначеЕсли ТипЗнч(ТабличныйДокумент) = Тип("ТекстовыйДокумент") Тогда
			ПрочитатьДанныеИзТекстовогоДокумента(ПротоДанные, ТабличныйДокумент);
		КонецЕсли;
	КонецЦикла;
...
Функция ПрочитатьДанныеИзТекстовогоДокумента(ПротоДанные, ТабличныйДокумент)

	Чтение = Новый ЧтениеJSON;
	Чтение.УстановитьСтроку(ТабличныйДокумент.ПолучитьТекст());
	ПротоДанныеКопия = ПрочитатьJSON(Чтение, Ложь);
	Для каждого Ключ из ПротоДанныеКопия Цикл
		ОписаниеОбъекта = СоздатьСтруктуруОписанияОбъекта();
		Для каждого КлючОбъекта из Ключ.Значение Цикл
			
			
			
			Значение = КлючОбъекта.Значение;
			Если КлючОбъекта.Ключ = "ПоляОбъекта" Тогда 
				Значение = МассивСтруктурВТаблицу(Значение, СоздатьТаблицуОписанияПолей());
			КонецЕсли;
			...
			ОписаниеОбъекта[КлючОбъекта.Ключ] = Значение
		КонецЦикла;
		
		ПротоДанные.Вставить(Ключ.Ключ, ОписаниеОбъекта);		
	КонецЦикла;
...
Функция ПолучитьОбъект(ПротоДанные,ПротоОбъект)
	...
	ИначеЕсли ПротоОбъект.Тип = ВернутьЗначениеСоответствия("РегистрРасчета") Тогда
		ОбъектСсылка = СоздатьОбъект_Регистр(РегистрыРасчета, ПротоДанные,ПротоОбъект);		
...
Функция СоздатьОбъект_Справочник(ПротоДанные,ПротоОбъект)
	...
	Исключение
		Ошибка = ОписаниеОшибки();
		Если РежимЭтоГруппа Тогда
			Сообщение = "Справочник."+ПротоОбъект.Вид+" не удалось создать новую группу.";
		Иначе
			Сообщение = "Справочник."+ПротоОбъект.Вид+" не удалось создать новый элемент.";
		КонецЕсли;
	...
	Если Объект = Неопределено Тогда
		...
		Если ПротоОбъект.Режимы.Предопределенный Тогда
			// если предопределенный элемент еще не инициализирован, то ссылки еще нет и при обращении по имени предопределенного выдается "ошибка не удалось найти предопределенный элемент/группу [ИмяПредопределенного]"
			Попытка
				СсылкаПредопределенного = СпрМенеджер[ПротоОбъект.ИмяПредопределенного];
			Исключение
	...
	Если Объект <> НовыйПустойОбъект Тогда
		ПропускаемыеРеквизиты = ПолучитьПропускаемыеРеквизиты(ПротоОбъект.Объект.Метаданные(), РежимЭтоГруппа);
...
Функция СоздатьОбъект_Регистр(МенеджерРегистра, ПротоДанные, ПротоОбъект)
	...
	ЭтоРегистрРасчета = МенеджерРегистра = РегистрыРасчета;
	...
	ИначеЕсли ЭтоРегистрРасчета Тогда
		ПредставлениеМдМенеджераРегистра = "РегистрыРасчета";		
	Иначе
		...
		Для каждого СтрокаЗаписи Из ПротоНаборЗаписей Цикл
			...
			Если РегистрСведенийПодчиненРегистратору Тогда
				...
				Если ЭтоРегистрРасчета Тогда
					Если НЕ ЗначениеЗаполнено(ЗаписьНабора.ПериодРегистрации) Тогда
						ЗаписьНабора.ПериодРегистрации = ЗаписьНабора.Регистратор.Дата;
					КонецЕсли;
				Иначе
...
Функция СоздатьОбъект_ПВХ(ПротоДанные, ПротоОбъект)
	...
	Исключение
		Ошибка = ОписаниеОшибки();
		Если РежимЭтоГруппа Тогда
			Сообщение = "ПланВидовХарактеристик."+ПротоОбъект.Вид+" не удалось создать новую группу.";
		Иначе
			Сообщение = "ПланВидовХарактеристик."+ПротоОбъект.Вид+" не удалось создать новый элемент.";
		КонецЕсли;
		ВызватьИсключение Сообщение + Символы.ПС + Ошибка;
	КонецПопытки;
...
Функция СоздатьОбъект_ПланВидовРасчетов(ПротоДанные, ПротоОбъект)
	...
	Если Объект=Неопределено Тогда
		...
		ИначеЕсли ПротоОбъект.Режимы.Группа Тогда
			Попытка
                ВызватьИсключение "Недопустимый режим ""Группа"" для ПланаВидовРасчета";
                Объект = СпрМенеджер.СоздатьГруппу();
			Исключение
				Ошибка = ОписаниеОшибки();
		...
		Иначе
			Попытка
				Объект = СпрМенеджер.СоздатьВидРасчета();
			Исключение
				Ошибка = ОписаниеОшибки();
	...
	//Служебные поля и значения
	Если Не ЗначениеЗаполнено(Объект.Код) тогда
		//Объект.УстановитьНовыйКод();  // У ПланВидовРасчетаОбъект.<Имя плана видов расчета> нет такого метода.
	КонецЕсли;
...
Функция ОчиститьОбъект(Знач Объект, Знач НовыйПустойОбъект, ПропускаемыеРеквизиты)

	Рез = Объект;
	Если Рез = Неопределено или Рез = НовыйПустойОбъект Тогда
		Рез = НовыйПустойОбъект;
	Иначе
		СтрокаРеквизитов = "";
		ЕстьПропускаемыеРеквизиты = ЗначениеЗаполнено(ПропускаемыеРеквизиты);
		Если ЕстьПропускаемыеРеквизиты Тогда
			Для каждого Реквизит Из ПропускаемыеРеквизиты Цикл
				СтрокаРеквизитов = СтрокаРеквизитов + Реквизит + ",";	
			КонецЦикла;
		...
		Для каждого мдТабЧасть из Рез.Метаданные().ТабличныеЧасти Цикл
			Если ЕстьПропускаемыеРеквизиты И ПропускаемыеРеквизиты.Найти(мдТабЧасть.Имя) = Неопределено Тогда
				Рез[мдТабЧасть.Имя].Очистить();
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьПропускаемыеРеквизиты(Знач Мд, Знач ИсключатьДляГруппы = Ложь)
	...
	Если КорневойТипКонфигурации = ВернутьЗначениеСоответствия("Справочник") Тогда
		...
		РезМассив.Добавить("Ссылка");
		Если Мд.Владельцы.Количество() = 0 Тогда
			РезМассив.Добавить("Владелец");
		КонецЕсли;
		Если Не Мд.Иерархический Тогда
			РезМассив.Добавить("Родитель");
		КонецЕсли;
        Если ЕстьСтандартныйРеквизит(Мд.СтандартныеРеквизиты, "ИмяПредопределенныхДанных") Тогда
            РезМассив.Добавить("ИмяПредопределенныхДанных");
        КонецЕсли; 
		
		Для каждого мдРеквизит из Мд.Реквизиты Цикл
			Если ИсключатьДляГруппы И мдРеквизит.Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляЭлемента Тогда
				РезМассив.Добавить(мдРеквизит.Имя);
			ИначеЕсли Не ИсключатьДляГруппы И мдРеквизит.Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляГруппы Тогда
				РезМассив.Добавить(мдРеквизит.Имя);
			КонецЕсли;
		КонецЦикла;				
		Для каждого мдТабЧасть из Мд.ТабличныеЧасти Цикл
			Если ИсключатьДляГруппы И мдТабЧасть.Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляЭлемента Тогда
				РезМассив.Добавить(мдТабЧасть.Имя);
			ИначеЕсли Не ИсключатьДляГруппы И мдТабЧасть.Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляГруппы Тогда
				РезМассив.Добавить(мдТабЧасть.Имя);
			КонецЕсли;
		КонецЦикла;
	...
	ИначеЕсли КорневойТипКонфигурации = ВернутьЗначениеСоответствия("РегистрРасчета") Тогда 
	...
	ИначеЕсли КорневойТипКонфигурации = ВернутьЗначениеСоответствия("ПланВидовХарактеристик") Тогда 
        ...
        Если ЕстьСтандартныйРеквизит(Мд.СтандартныеРеквизиты, "ИмяПредопределенныхДанных") Тогда
            РезМассив.Добавить("ИмяПредопределенныхДанных");
        КонецЕсли; 
		РезМассив.Добавить("ТипЗначения");
...
Функция ПолучитьЗначениеПоля(ПротоДанные,стрПоле,НаборМдРеквизитов)
	...
	Если ПустаяСтрока(стрПоле.Режим) Тогда
		...
		Иначе
			Если ЗначениеЗаполнено(НаборМдРеквизитов) И НаборМдРеквизитов[стрПоле.Имя].Тип = Новый ОписаниеТипов("ХранилищеЗначения") И 
					стрПоле.Значение <> "ХранилищеЗначения" Тогда
...
Функция ВычислитьЗначениеПоТипуПоля(НаборМдРеквизитов,стрПоле)
	...
	Попытка
		...
		Если Не УжеВычислилиЗначение и ЕстьМетаданныеРеквизита Тогда
			...
			Если ЗначениеЗаполнено(стрПоле.Значение) И НЕ стрПоле.Значение="00000000-0000-0000-0000-000000000000" И Не ЗначениеЗаполнено(Значение) Тогда // пустая ссылка это не ошибка
				ВызватьИсключение "ЗначениеНеНайдено: Реквизит <"+стрПоле.Имя+">: Искали значение <"+стрПоле.Значение+">, но не нашли!";
			КонецЕсли;
		КонецЕсли;
	Исключение
...
Функция СоздатьМакетДляГенерацииДанных(МассивДанных) Экспорт
	...
	Для каждого ОписательОбъекта Из МассивДанных Цикл
		...
		Иначе
			...
			ОписаниеОбъектаВыгрузки = СоздатьСтруктуруОписанияОбъекта();
...
Функция СоздатьОписательЭлементаПоСсылке(Ссылка, МассивИмен, ДанныеОбъекта = Неопределено) Экспорт
	...
	ИначеЕсли Метаданные.ПланыВидовХарактеристик.Содержит(МетаСсылка) И Ссылка.ЭтоГруппа Тогда
		ОписательЭлемента.РежимСоздания = "Группа";
...
Функция СоздатьНовыйОбъект(Знач Ссылка, Знач Мд, Знач ЭтоГруппа = Ложь)
	Попытка
		лПустойОбъект = Неопределено;
		менеджерОбъекта = ПолучитьМенеджерОбъекта_ирЛкс(Мд);
		КорневойТипКонфигурации = ПолучитьКорневойТипКонфигурации_ирЛкс(Мд);
		Если КорневойТипКонфигурации = ВернутьЗначениеСоответствия("Справочник") Тогда
			Если ЭтоГруппа Или ЗначениеЗаполнено(Ссылка) и Ссылка.ЭтоГруппа Тогда
		...
		Возврат лПустойОбъект;

	Исключение
		ИнфоОшибки = ИнформацияОбОшибке();
		
		ОписаниеОшибки = "Возможно, у пользователя нет хватает прав на создание нового пустого элемента для типа <" + КорневойТипКонфигурации + ">.
			|
			|Данное право нужно для правильного создания макета данных. 
			|Обратитесь к администратору ИБ 1С для получения необходимого прав на создание нового элемента!
			|
...
Функция ВыгружаемыеСтандартныеРеквизиты(ОписательОбъекта, КорневойТипКонфигурации)
	...
	Если КорневойТипКонфигурации = ВернутьЗначениеСоответствия("Справочник") Тогда
		...
		Если МетаданныеСправочника.Иерархический Тогда
			...
			ВыгружаемыеСтандартныеРеквизиты.Добавить(ВернутьЗначениеСоответствия("ЭтоГруппа"));
		КонецЕсли;
	...
	ИначеЕсли КорневойТипКонфигурации = ВернутьЗначениеСоответствия("РегистрНакопления") 
		или КорневойТипКонфигурации = ВернутьЗначениеСоответствия("РегистрБухгалтерии") 
		или КорневойТипКонфигурации = ВернутьЗначениеСоответствия("РегистрРасчета") Тогда
...
Процедура ОбработатьРежимыОбъекта(Макет, ОписательОбъекта, ШапкаДанных, МассивДанных, Объект)
	...
	Если Метаданные.Справочники.Содержит(Мета) И Объект.ЭтоГруппа
		Или Метаданные.ПланыВидовХарактеристик.Содержит(Мета) И Объект.ЭтоГруппа Тогда
...
Процедура ДобавитьПростойРеквизитВМакет(Макет, ШапкаДанных, ИмяРеквизита, ПредставлениеЗначенияРеквизита)
	...
	Если СтрЧислоСтрок(ПредставлениеЗначенияРеквизита) > 1 Тогда
		ОбластьМакета_Значение.ВысотаСтроки = 10;
	КонецЕсли; 
...
Процедура ПолучитьПредставлениеИРежимПоискаПоЗначениюРеквизита(Объект, ЗначениеРеквизита, ТипРеквизита, МассивДанных, резПредставлениеЗначенияРеквизита, резРежимПоиска, резДополнительныйТипЗначения)
	...
	ИначеЕсли ЛиСсылкаНаОбъектБД_ирЛкс(ЗначениеРеквизита) Тогда
		...
		Если ОписательЗначенияРеквизита <> Неопределено Тогда
			...
			// нужно записать ДополнительныйТипЗначения для ссылки составного типа, если она среди выгружаемых объектов
			// иначе ошибка при загрузке
			Если ЕстьРеквизитСсылка Тогда
	...
	ИначеЕсли ТипРеквизита = Тип("ХранилищеЗначения") Тогда
		ПредставлениеЗначенияРеквизита = XMLСтрока(ЗначениеРеквизита);
	Иначе
		УжеВычислилиЗначение = Ложь;
...
Процедура ПроверитьЗагрузкуМакета(ТабличныйДокумент,СохранитьВБазе=Ложь) Экспорт
	НачатьТранзакцию();
	...
	Если ТранзакцияАктивна() Тогда  // применяем изменения в базе
		Если СохранитьВБазе=Ложь Тогда
			ОтменитьТранзакцию();
		Иначе
...
Процедура ЗаполнитьСоответствие()
	...
	АнглийскоеСоответствие.Вставить("РегистрРасчета",			"CalculationRegister");	
	...
	АнглийскоеСоответствие.Вставить("РегистрНакопленияНаборЗаписей",		"AccumulationRegisterRecordSet");
	АнглийскоеСоответствие.Вставить("РегистрБухгалтерииНаборЗаписей",		"AccountingRegisterRecordSet");
	АнглийскоеСоответствие.Вставить("РегистрРасчетаНаборЗаписей",			"CalculationRegisterRecordSet");
КонецПроцедуры
