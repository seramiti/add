...
Процедура ВосстановитьНастройкиКлиент() Экспорт
	...
	Если СтрЗапуска <> "" Тогда
		...
		StartFeaturePlayer = Настройки.ПолучитьЗначениеПереданногоПараметра(СтруктураПараметров, "StartFeaturePlayer");
		...
		NoLoadConfig = Настройки.ПолучитьЗначениеПереданногоПараметра(СтруктураПараметров, "NoLoadConfig");
		...
		LibsOnly = Настройки.ПолучитьЗначениеПереданногоПараметра(СтруктураПараметров, "LibsOnly");
...
Процедура ДобавитьСнипет(Знач Снипет, Знач ИмяФайла, ВерсияФайла, БылиОшибки, ТзнТаблицаИзвестныхStepDefinition, 
	...
	Если ПромСтр <> Неопределено Тогда
		...
		//Сообщить(ТекстСообщения);
		//Возврат; //FIXME: почему закоментированно...
	КонецЕсли;
...
Процедура ДобавитьСнипетыСервер(МассивСнипетовИзОбработки, ИмяФайла, ВерсияФайла, БылиОшибки, МассивВажныхСообщений)
	...
	Для каждого Снипет Из МассивСнипетовИзОбработки Цикл
		ДобавитьСнипет(Снипет, ИмяФайла, ВерсияФайла, БылиОшибки, ТзнТаблицаИзвестныхStepDefinition, МассивВажныхСообщений);
	КонецЦикла;
...
Процедура ПриОткрытииЗавершение(Знач НенужноеЗначение, Знач ДополнительныеПараметры) Экспорт
	...
	ТаймайтЗагрузкиНастроек = ?(Объект.ПлагиныЗагружены, 0.1, 5);
...
Процедура ПроставитьИДРекурсивно(ДеревоСтроки, ИД, МассивСтрокДереваДанныеФормы, МассивИменКонтекстовОбработок, ТзнТаблицаИзвестныхStepDefinition, ДопПараметры, ЕстьСвояEPFУФичи = Неопределено)
	Для каждого СтрокаДерева Из ДеревоСтроки Цикл
		...
		Если СтрокаДерева.Тип = "Фича" Тогда
			...
			ФайлEPF  = УниверсальноеПолноеИмяФайлаШаговФичи_Устаревший(ФайлФичи.Путь, ФайлФичи.ИмяБезРасширения, ДопПараметры.ЭтоLinux);
...
Процедура ВызватьМетодСценарияЕслиОнЕсть(СтрокаДереваСценария, ИмяПроцедуры, МассивПараметров = Неопределено,
	...
	Если ИДМассивИменКонтекстовОбработок = Неопределено Тогда //значит у фичи нет своей epf
		АдресСнипета_Устаревший = УниверсальноеПолноеИмяФайлаШаговФичи_Устаревший(ДополнитьСлешВПуть(КаталогФичи), Файл.ИмяБезРасширения, ЭтоLinux);
		АдресСнипета_Устаревший = УниверсальноеПолноеИмяФайла(АдресСнипета_Устаревший);

		ИДМассивИменКонтекстовОбработок = МассивИменКонтекстовОбработок.Найти(УниверсальноеПолноеИмяФайла(АдресСнипета_Устаревший, Истина));
		Если ИДМассивИменКонтекстовОбработок = Неопределено Тогда //значит у фичи нет своей epf
			Возврат;
		КонецЕсли;
...
Функция РаспаковатьEPF(СтруктураОписанияEpf)
	...
	Попытка
		...
		КомандаСистемы = """" + ДополнитьСлешВПуть(Объект.ВерсияПлатформыДляГенерацииEPF) + "1cv8"" DESIGNER /F """ + ВременныйКаталогБазы + """ /DumpExternalDataProcessorOrReportToFiles  """ + ПутьКИсходникам + """ """ + ИмяФайла + """ /out """ + ИмяФайлаOUT + """ > """ + ИмяФайлаЛога + """";
		Отладка(КомандаСистемы);
		ВыполнитьКомандуОСБезПоказаЧерногоОкна(КомандаСистемы);
	...
	Если GenerateEpf Тогда
		...
		Если ФайлСуществует Тогда //т.е. если мы не первый раз работаем с данной фичей
			Если НЕ Объект.ВыводитьТекстМодуляВместоПерегенерацииEPF Тогда
				СтруктураОписанияEpf_Копия = Новый ФиксированнаяСтруктура(СтруктураОписанияEpf);

				РаспаковатьEPF(СтруктураОписанияEpf);
...
&НаКлиенте
Процедура Редактор_VSCode(Команда)
	Объект.КоманднаяСтрокаОткрытияФичи = "code -g %FeatureFile%:%FeatureLine%";
КонецПроцедуры
